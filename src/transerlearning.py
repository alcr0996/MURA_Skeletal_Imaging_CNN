...
if __name__ == "__main__":
    ...
    model.add(Conv2D(32, (3, 3), padding='same', name = 'fourth_cnn_layer'))
    model.add(Activation(LeakyReLU(), name = 'fourth_cnn_activation'))
    model.add(MaxPooling2D(pool_size=(2, 2), name = 'fourth_pooling_layer'))

    model.add(Flatten())

    model.add(Dense(256, name = 'second_dense_layer'))
    model.add(Activation(LeakyReLU(), name = 'second_dense_activation'))
    model.add(Dropout(0.25, name = 'second_dense_dropout'))
